(function(){
  ...
  function getCompletedLogs(){
    const logs = window.MBSanmaMatchLog && typeof window.MBSanmaMatchLog.getStoredLogs === "function"
      ? window.MBSanmaMatchLog.getStoredLogs()
      : [];
    return Array.isArray(logs) ? logs.filter((item)=> item && typeof item === "object" && item.endedAt) : [];
  }

  function loadTargetMatch(){
    const normalizer = window.MBSanmaLogNormalizer;
    const logs = getCompletedLogs();
    const targetId = getQueryMatchId();
    let raw = logs.find((item)=> item && item.matchId === targetId) || logs[0] || null;
  ...
  function boot(){
    ...
    state.match = loadTargetMatch();

    if (!state.match){
      if (matchSummaryCard) matchSummaryCard.innerHTML = '<div class="emptyCard">表示できる半荘ログがありません。一覧へ戻って確認してください。</div>';
      if (currentCard) currentCard.innerHTML = '<div class="emptyCard">半荘ログが見つかりません。</div>';
      if (startInfoList) startInfoList.innerHTML = '<div class="emptyCard">開始情報がありません。</div>';
      if (trailList) trailList.innerHTML = '<div class="emptyCard">表示できるイベントがありません。</div>';
      if (eventLogList) eventLogList.innerHTML = '<div class="emptyCard">ログがありません。</div>';
