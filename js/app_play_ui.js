...
(function(){
  ...
  function buildLogsPanel(logs){
    ...
    note.innerHTML = "最新の半荘ログを<strong>見やすい単位にまとめて</strong>表示します。CPUの候補・判断・実行は1行に圧縮し、局内では直近のまとまりだけを追えるようにしています。";
    ...
    logs.slice(0, 5).forEach((log, index)=>{
      ...
      title.textContent = `${index === 0 ? "最新" : `${index + 1}件前`} / ${formatLogDateTime(log.startedAt)}`;
      const meta = document.createElement("div");
      meta.className = "visitorLogMatchMeta";
      const kyokuCount = Array.isArray(log && log.kyokus) ? log.kyokus.length : 0;
      meta.innerHTML = `matchId: ${log && log.matchId ? log.matchId : "—"}<br>局数: ${kyokuCount} / ${summarizeLogSummary(log)}`;
      left.appendChild(title);
      ...
      kyokus.forEach((kyoku, kyokuIndex)=> kyokuList.appendChild(buildLogKyokuBlock(kyoku, kyokuIndex)));
  ...
  function renderStatsOverlay(){
    ...
    const tabButtons = [];
    const panelEls = [];
    const availableTabKeys = history.length ? ["overview", "graphs", "detail", "logs"] : ["logs"];
