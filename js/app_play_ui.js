...
(function(){
  ...
  function normalizeStatsTabKey(value){
    ...
    return ["overview", "graphs", "detail", "logs"].includes(key) ? key : "overview";
  }
  ...
  function renderStatsOverlay(){
    ...
    const normalizer = (typeof window !== "undefined" && window.MBSanmaLogNormalizer) ? window.MBSanmaLogNormalizer : null;
    const normalizedLogs = normalizer && typeof normalizer.normalizeStoredLogs === "function"
      ? normalizer.normalizeStoredLogs(storedLogs)
      : [];
    const logMetricsApi = (typeof window !== "undefined" && window.MBSanmaLogMetrics) ? window.MBSanmaLogMetrics : null;
    const logSummary = logMetricsApi && typeof logMetricsApi.summarizeLogs === "function"
      ? logMetricsApi.summarizeLogs(normalizedLogs)
      : null;
    ...
    const tabButtons = [];
    const panelEls = [];
    const availableTabKeys = hasHistory
      ? ["overview", "graphs", "detail", "logs"]
      : ["overview", "logs"];
    const normalizedActiveKey = normalizeStatsTabKey(activeStatsTabKey);
    const activeKey = availableTabKeys.includes(normalizedActiveKey) ? normalizedActiveKey : availableTabKeys[0];
    ...
    if (hasHistory){
      ...
      const note = document.createElement("div");
      note.className = "visitorStatsEmpty";
      note.innerHTML = "半荘保存の成績はまだありません。<br>いまは対局ログから暫定集計を表示しています。";
      overviewPanel.appendChild(note);

      if (logSummary){
        const shadowAgreeRateText = (()=>{
          const comparable = Number(logSummary.shadowComparableCount) || 0;
          if (comparable <= 0) return "—";
          return `${((Number(logSummary.shadowAgreeCount) || 0) / comparable * 100).toFixed(1)}%`;
        })();

        const reasonTagEntries = Object.entries(logSummary.reasonTagCounts || {})
          .sort((a, b)=> (Number(b[1]) || 0) - (Number(a[1]) || 0))
          .slice(0, 3)
          .map(([key, count])=> `${key}×${count}`);

        overviewPanel.appendChild(buildStatsMetricPanel("ログ暫定集計", [
          { label: "対局ログ", value: `${Number(logSummary.matchCount) || 0}`, hint: "保存中の半荘ログ数" },
          { label: "局数", value: `${Number(logSummary.kyokuCount) || 0}`, hint: "局ログ総数" },
          { label: "表示行", value: `${Number(logSummary.rowCount) || 0}`, hint: "正規化後の行数" },
          { label: "生イベント", value: `${Number(logSummary.rawEventCount) || 0}`, hint: "保存されたイベント総数" },
          { label: "あなた打牌", value: `${Number(logSummary.playerDiscardCount) || 0}`, hint: "プレイヤー打牌" },
          { label: "リーチ", value: `${Number(logSummary.playerRiichiCount) || 0}`, hint: "ログ基準" },
          { label: "北抜き", value: `${Number(logSummary.playerPeiCount) || 0}`, hint: "ログ基準" },
          { label: "アガリ", value: `${Number(logSummary.playerAgariCount) || 0}`, hint: "ツモ+ロン" },
          { label: "放銃", value: `${Number(logSummary.playerHojuCount) || 0}`, hint: "ロン放銃" },
          { label: "副露", value: `${Number(logSummary.playerOpenCallCount) || 0}`, hint: "ポン/明槓/加槓" }
        ]));

        overviewPanel.appendChild(buildStatsMetricPanel("CPU判断ログ", [
          { label: "CPU打牌判断", value: `${Number(logSummary.cpuDiscardCount) || 0}`, hint: "CPU discard rows" },
          { label: "CPU副露判断", value: `${Number(logSummary.cpuOpenCount) || 0}`, hint: "CPU open rows" },
          { label: "shadow一致率", value: shadowAgreeRateText, hint: "比較可能行のみ" },
          { label: "external採用", value: `${Number((logSummary.executionSourceCounts && logSummary.executionSourceCounts.external) || 0)}`, hint: "CPU打牌" },
          { label: "internal採用", value: `${Number((logSummary.executionSourceCounts && logSummary.executionSourceCounts.internal) || 0)}`, hint: "CPU打牌" },
          { label: "legacy採用", value: `${Number((logSummary.executionSourceCounts && logSummary.executionSourceCounts.legacy) || 0)}`, hint: "CPU打牌" },
          { label: "見送り", value: `${Number(logSummary.callPassCount) || 0}`, hint: "call_pass" },
          { label: "鳴き候補", value: `${Number(logSummary.callPromptCount) || 0}`, hint: "call_prompt" },
          { label: "主要reason", value: reasonTagEntries.length ? reasonTagEntries.join(" / ") : "—", hint: "上位3件" }
        ]));
      }

      panelEls.push(overviewPanel);
      panels.appendChild(overviewPanel);
    }
