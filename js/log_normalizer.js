...
(function(global){
  ...
  function buildCpuDiscardEntry(bundle){
    ...
    if (bundle.codeMismatch && bundle.codeMismatch.expectedCode && bundle.codeMismatch.actualCode){
      detailBits.push(`actual=${bundle.codeMismatch.actualCode}`);
    }
      ...
      if (event.type === "cpu_discard_snapshot"){
        ...
        for (let j = i + 1; j < list.length; j++){
          ...
          if (next.type === "discard" && nextPayload.seatIndex === seatIndex){
            ...
            bundle.rawCount += 1;
            used.add(j);
            if (expectedCode && actualCode && expectedCode !== actualCode && !bundle.codeMismatch){
              bundle.codeMismatch = { expectedCode, actualCode };
            }
            break;
          }
        }
