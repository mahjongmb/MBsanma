...
(function(global){
  ...
  function normalizeEventType(type){
    ...
    if (raw === "call_pon") return "pon";
    if (raw === "call_minkan") return "minkan";
    return raw;
  }
  ...
  function getDiscardCode(payload){
    return payload.tile && payload.tile.code ? payload.tile.code : (payload.code || "?");
  }

  function buildCallPromptDetail(payload){
    const actions = [];
    if (payload.canRon || payload.action === "ron") actions.push("ロン");
    if (payload.canPon || payload.action === "pon") actions.push("ポン");
    if (payload.canMinkan || payload.action === "minkan") actions.push("明槓");
    ...
    const tileCode = payload.discardedTile && payload.discardedTile.code
      ? payload.discardedTile.code
      : (payload.tile && payload.tile.code ? payload.tile.code : (payload.code || ""));
    const bits = [];
    if (tileCode) bits.push(`対象=${tileCode}`);
    if (actions.length) bits.push(`候補=${actions.join("/")}`);
    return bits.join(" / ");
  }

  function buildDefaultEntry(event){
    const payload = getPayload(event);
    const rawType = event && event.type ? event.type : "event";
    ...
    if (type === "draw"){
      title = `${getSeatLabel(seatIndex)} ツモ ${payload.tile && payload.tile.code ? payload.tile.code : "?"}`;
      if (payload.source) detail = `source=${payload.source}`;
    }else if (type === "discard"){
      const code = getDiscardCode(payload);
      title = `${getSeatLabel(seatIndex)} 打牌 ${code}`;
      if (payload.isRiichiDeclare || (payload.tile && payload.tile.isRiichiDeclare)) extra = "リーチ宣言牌";
    }else if (type === "riichi"){
      title = `${getSeatLabel(seatIndex)} リーチ`;
      const code = getDiscardCode(payload);
      if (code && code !== "?") detail = `宣言牌=${code}`;
    }else if (type === "pei"){
      title = `${getSeatLabel(seatIndex)} 北抜き`;
    }else if (type === "pon"){
      title = `${getSeatLabel(seatIndex)} ポン ${payload.code || (payload.tile && payload.tile.code) || "?"}`;
    }else if (type === "minkan"){
      title = `${getSeatLabel(seatIndex)} 明槓 ${payload.code || (payload.tile && payload.tile.code) || "?"}`;
    }else if (type === "ankan"){
      title = `${getSeatLabel(seatIndex)} 暗槓 ${payload.code || (payload.tile && payload.tile.code) || "?"}`;
    }else if (type === "kakan"){
      title = `${getSeatLabel(seatIndex)} 加槓 ${payload.code || (payload.tile && payload.tile.code) || "?"}`;
    }else if (type === "call_prompt"){
      title = `${getSeatLabel(seatIndex == null ? 0 : seatIndex)} 鳴き候補`;
      detail = buildCallPromptDetail(payload);
    }else if (type === "call_pass"){
      title = `${getSeatLabel(seatIndex == null ? 0 : seatIndex)} 見送り`;
      detail = buildCallPromptDetail(payload);
      ...
      title = `${winners} ロン`;
      if (Number.isInteger(payload.discarderSeatIndex)){
        detail = `放銃=${getSeatLabel(payload.discarderSeatIndex)}`;
      }
    ...
    return {
      ...
      rawCount: 1,
      seatIndex,
      winnerSeatIndex: Number.isInteger(payload.winnerSeatIndex) ? payload.winnerSeatIndex : null,
      winnerSeatIndexes: Array.isArray(payload.winnerSeatIndexes) ? payload.winnerSeatIndexes.slice() : [],
      discarderSeatIndex: Number.isInteger(payload.discarderSeatIndex) ? payload.discarderSeatIndex : null,
      isRiichiDeclare: !!(payload.isRiichiDeclare || (payload.tile && payload.tile.isRiichiDeclare))
    };
  }
  ...
  function buildCpuDiscardEntry(bundle){
    ...
    return {
      ...
      shadowAgree: lastDecisionPayload.shadowAgree === true ? true : (lastDecisionPayload.shadowAgree === false ? false : null)
    };
  }
