...
function getRiverRefBySeatIndexForLog(seatIndex){
...
function getSeatJunmeFromStateForLog(seatIndex){
  const riverRef = getRiverRefBySeatIndexForLog(seatIndex);
  return Math.max(1, Array.isArray(riverRef) ? riverRef.length : 1);
}

function addVisibleTileCountByCodeForRiichi(out, tilesLike){
  if (!out || typeof out !== "object" || !Array.isArray(tilesLike)) return;
  for (const tile of tilesLike){
    if (!tile || !tile.code) continue;
    out[tile.code] = (Number(out[tile.code]) || 0) + 1;
  }
}

function addVisibleMeldCountByCodeForRiichi(out, meldList){
  if (!out || typeof out !== "object" || !Array.isArray(meldList)) return;
  for (const meld of meldList){
    if (!meld || !meld.code) continue;
    const kind = meld.type || "pon";
    const amount = (kind === "ankan" || kind === "minkan" || kind === "kakan") ? 4 : 3;
    out[meld.code] = (Number(out[meld.code]) || 0) + amount;
  }
}

function buildPlayerVisibleCountsByCodeForRiichi(afterHand13, declareTile){
  const out = {};
  addVisibleTileCountByCodeForRiichi(out, afterHand13);
  addVisibleTileCountByCodeForRiichi(out, river);
  addVisibleTileCountByCodeForRiichi(out, cpuRightRiver);
  addVisibleTileCountByCodeForRiichi(out, cpuLeftRiver);
  addVisibleTileCountByCodeForRiichi(out, peis);
  try{ if (Array.isArray(cpuRightPeis)) addVisibleTileCountByCodeForRiichi(out, cpuRightPeis); }catch(e){}
  try{ if (Array.isArray(cpuLeftPeis)) addVisibleTileCountByCodeForRiichi(out, cpuLeftPeis); }catch(e){}
  if (declareTile && declareTile.code){
    out[declareTile.code] = (Number(out[declareTile.code]) || 0) + 1;
  }
  if (Array.isArray(doraIndicators)) addVisibleTileCountByCodeForRiichi(out, doraIndicators);
  addVisibleMeldCountByCodeForRiichi(out, melds);
  try{ if (Array.isArray(cpuRightMelds)) addVisibleMeldCountByCodeForRiichi(out, cpuRightMelds); }catch(e){}
  try{ if (Array.isArray(cpuLeftMelds)) addVisibleMeldCountByCodeForRiichi(out, cpuLeftMelds); }catch(e){}
  return out;
}

function buildPlayerRiichiLogPayload(declareTile, extra){
  const payload = {
    seatIndex: 0,
    junme: getSeatJunmeFromStateForLog(0),
    ...
    riichiKind: (typeof isDoubleRiichiSeat === "function" && isDoubleRiichiSeat(0)) ? "double" : "normal",
    source: extra && extra.source ? extra.source : "",
    turnPhase: extra && extra.turnPhase ? extra.turnPhase : ""
  };

  try{
    if (typeof window !== "undefined" && window.MBSanmaMatchLog && typeof window.MBSanmaMatchLog.buildTenpaiAnalysisFromTiles === "function"){
      const afterHand13 = Array.isArray(hand13) ? hand13.slice() : [];
      const visibleCountsByCode = buildPlayerVisibleCountsByCodeForRiichi(afterHand13, declareTile);
...
function pushPlayerRiichiDeclareLogSafe(declareTile, extra){
  try{
    if (typeof window === "undefined" || !window.MBSanmaMatchLog || typeof window.MBSanmaMatchLog.pushEvent !== "function") return;
    window.MBSanmaMatchLog.pushEvent("riichi", buildPlayerRiichiLogPayload(declareTile, extra));
  }catch(e){}
}
...
function discardFromHand13(idx){
  ...
  if (isRiichiDeclareDiscard){
    pushPlayerRiichiDeclareLogSafe(t, {
...
function discardDrawn(allowRiichiTsumoSkip = false){
  ...
  if (isRiichiDeclareDiscard){
    pushPlayerRiichiDeclareLogSafe(discardedDrawnTile, {
      source: "drawn",
      turnPhase: (typeof turnPhase !== "undefined") ? turnPhase : ""
    });
  }
