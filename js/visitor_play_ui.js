...
(function(){
  ...
  function injectVisitorOverlayStyles(){
    ...
    style.textContent = `
      ...
      .visitorStatsPreviewText{
        font-size: 13px;
        line-height: 1.65;
        color: rgba(245,247,244,0.88);
      }
      .visitorStatsPreviewText strong{
        color: #ffd56f;
      }
  ...
  function renderStatsOverlay(){
    ...
    const useSampleHistory = !hasRealHistory || statsPreviewMode;
    const history = useSampleHistory ? buildSampleHistory() : realHistory;

    if (useSampleHistory){
      const previewBar = document.createElement("div");
      previewBar.className = "visitorStatsPreviewBar";

      const previewText = document.createElement("div");
      previewText.className = "visitorStatsPreviewText";
      if (hasRealHistory){
        previewText.innerHTML = "<strong>確認用サンプル</strong>を表示中です。見た目調整の確認用なので、実データには影響しません。";
      }else{
        previewText.innerHTML = "まだ実データが無いので、<strong>確認用サンプル</strong>を表示しています。半荘終了後は実データが保存されます。";
      }
      previewBar.appendChild(previewText);

      const previewActions = document.createElement("div");
      previewActions.className = "visitorStatsPreviewActions";

      if (hasRealHistory){
        const toggleBtn = document.createElement("button");
        toggleBtn.type = "button";
        toggleBtn.className = "visitorStatsPreviewBtn";
        toggleBtn.textContent = "実データに戻す";
        toggleBtn.addEventListener("click", ()=>{
          statsPreviewMode = false;
          renderStatsOverlay();
        });
        previewActions.appendChild(toggleBtn);
      }
      ...
      const previewText = document.createElement("div");
      previewText.className = "visitorStatsPreviewText";
      previewText.textContent = "直近3半荘の実データを表示中です。";
      previewBar.appendChild(previewText);
      ...
      toggleBtn.type = "button";
      toggleBtn.className = "visitorStatsPreviewBtn";
      toggleBtn.textContent = "サンプル表示";
      toggleBtn.addEventListener("click", ()=>{
        statsPreviewMode = true;
        renderStatsOverlay();
      });
      previewActions.appendChild(toggleBtn);
