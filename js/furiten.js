...
//
// UI
// - フリテン状態のとき、ツモボタンの横に薄く「フリテン」と表示する
//   ★actionbar(flex)に混ぜると崩れるので、body直下 fixed で追従させる
//
...
(function(){
  ...
  // =========================================================
  // UI：ツモボタン横「フリテン」バッジ（レイアウト非参加）
  // =========================================================
  function getTsumoBtnEl(){
    let btn = null;
    try{
      if (typeof tsumoBtn !== "undefined" && tsumoBtn) btn = tsumoBtn;
    }catch(e){}
    if (!btn) btn = document.getElementById("tsumoBtn");
    return btn || null;
  }
  ...
  function ensureFuritenBadge(){
    ...
    badge.style.border = "1px solid rgba(0,0,0,0.22)";
    badge.style.background = "rgba(255,255,255,0.35)";
    badge.style.color = "rgba(0,0,0,0.85)";
  ...
  function positionBadgeNearTsumo(badge, btn){
  ...
  function updateFuritenBadge(){
    try{
      ...
      positionBadgeNearTsumo(badge, btn);
    ...
    function wrapped(){
      try{
        if (F.enabled && typeof pendingCall !== "undefined" && pendingCall && pendingCall.canRon){
          ...
          F.temp = true;
          if (riichiNow) F.tempPermanent = true;
