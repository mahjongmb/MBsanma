...
function buildMatchLogStartMeta(){
  const runtimeMode = (typeof readAppRuntimeModeFromSessionStorage === "function")
    ? readAppRuntimeModeFromSessionStorage()
    : { launchMode: "battle", playerMode: "manual", isBattleMode: true, isVerifyMode: false, isAutoPlayer: false };

  const matchMode = (isAutoMatchBatchRunning() || runtimeMode.isVerifyMode)
    ? "cpu_batch"
    : "app_play";
  ...
  const meta = {
    ...
    appTitle: "MBサンマアプリ版",
    matchMode,
    launchMode: runtimeMode.launchMode || "battle",
    playerMode: runtimeMode.playerMode || "manual",
    playerControl: {
  ...
  if (isAutoMatchBatchRunning()){
    meta.batchRun = {
      currentIndex: (Number(autoMatchBatchState.completedCount) || 0) + 1,
      totalCount: Number(autoMatchBatchState.totalCount) || 0
    };
  }
...
function applyAppRuntimeModeToPlayerControls(){
  const config = readAppRuntimeModeFromSessionStorage();

  if (config.isVerifyMode || config.isAutoPlayer){
    setPlayerUnifiedControlMode("internal");
  } else {
    setPlayerUnifiedControlMode("manual");
  }
...
const AUTO_MATCH_BATCH_DEFAULT_COUNT = 100;
const AUTO_MATCH_BATCH_MAX_COUNT = 5000;
const AUTO_MATCH_BATCH_NEXT_HANCHAN_DELAY_MS = 260;

let autoMatchBatchRequestedCount = AUTO_MATCH_BATCH_DEFAULT_COUNT;
const autoMatchBatchState = {
  isRunning: false,
  totalCount: 0,
  completedCount: 0,
  stopRequested: false,
  previousPlayerControlModes: null,
  lastSummaryText: ""
};

function normalizeAutoMatchBatchCount(value){
  const n = Number(value);
  if (!Number.isFinite(n)) return AUTO_MATCH_BATCH_DEFAULT_COUNT;
  return Math.min(AUTO_MATCH_BATCH_MAX_COUNT, Math.max(1, Math.floor(n)));
}

function getAutoMatchBatchRequestedCount(){
  return normalizeAutoMatchBatchCount(autoMatchBatchRequestedCount);
}

function setAutoMatchBatchRequestedCount(value){
  autoMatchBatchRequestedCount = normalizeAutoMatchBatchCount(value);
  return autoMatchBatchRequestedCount;
}

function isAutoMatchBatchRunning(){
  return !!autoMatchBatchState.isRunning;
}

function getAutoMatchBatchStateSnapshot(){
  return {
    isRunning: !!autoMatchBatchState.isRunning,
    totalCount: Number(autoMatchBatchState.totalCount) || 0,
    completedCount: Number(autoMatchBatchState.completedCount) || 0,
    stopRequested: !!autoMatchBatchState.stopRequested,
    lastSummaryText: autoMatchBatchState.lastSummaryText || ""
  };
}

function buildPlayerControlModeSnapshot(){
  return {
...
function applyPlayerControlModeSnapshot(snapshot){
  const src = (snapshot && typeof snapshot === "object") ? snapshot : {};
  setPlayerDiscardControlMode(src.discardControlMode);
  setPlayerOpenControlMode(src.openControlMode);
  setPlayerSpecialControlMode(src.specialControlMode);
}

function syncAutoMatchBatchUi(){
  try{
...
function closeHanchanEndOverlayForAutoMatchBatch(){
  try{
...
function finishAutoMatchBatchRun(reason){
  const completed = Number(autoMatchBatchState.completedCount) || 0;
  const total = Number(autoMatchBatchState.totalCount) || 0;

  closeHanchanEndOverlayForAutoMatchBatch();

  if (autoMatchBatchState.previousPlayerControlModes){
    applyPlayerControlModeSnapshot(autoMatchBatchState.previousPlayerControlModes);
  }

  autoMatchBatchState.isRunning = false;
  autoMatchBatchState.totalCount = 0;
  autoMatchBatchState.completedCount = completed;
  autoMatchBatchState.stopRequested = false;
  autoMatchBatchState.previousPlayerControlModes = null;
  autoMatchBatchState.lastSummaryText = (reason === "stopped")
    ? `停止済み ${completed} / ${total}半荘`
    : `完了 ${completed} / ${total}半荘`;

  saveGameSettingsToStorage();
  syncQuickSettingButtons();
  syncAutoMatchBatchUi();
...
function handleAutoMatchBatchHanchanEnd(endInfo, settlement){
  if (!isAutoMatchBatchRunning()) return;

  autoMatchBatchState.completedCount = (Number(autoMatchBatchState.completedCount) || 0) + 1;
  syncAutoMatchBatchUi();

  const shouldStop = autoMatchBatchState.stopRequested
    || autoMatchBatchState.completedCount >= autoMatchBatchState.totalCount;

  setTimeout(()=>{
    closeHanchanEndOverlayForAutoMatchBatch();

    if (shouldStop){
      finishAutoMatchBatchRun(autoMatchBatchState.stopRequested ? "stopped" : "completed");
      return;
    }

    startNewHanchan();
  }, AUTO_MATCH_BATCH_NEXT_HANCHAN_DELAY_MS);
}

function startAutoMatchBatchRun(){
  if (isAutoMatchBatchRunning()) return false;

  const requestedCount = getAutoMatchBatchRequestedCount();
  autoMatchBatchState.isRunning = true;
  autoMatchBatchState.totalCount = requestedCount;
  autoMatchBatchState.completedCount = 0;
  autoMatchBatchState.stopRequested = false;
  autoMatchBatchState.previousPlayerControlModes = buildPlayerControlModeSnapshot();
  autoMatchBatchState.lastSummaryText = "";
...
function requestStopAutoMatchBatchRun(){
  if (!isAutoMatchBatchRunning()) return false;
  autoMatchBatchState.stopRequested = true;
  autoMatchBatchState.lastSummaryText = "停止予約中";
  syncAutoMatchBatchUi();
  return true;
}

function buildAutoMatchBatchSectionHtml(){
  const state = getAutoMatchBatchStateSnapshot();
  const requestedCount = getAutoMatchBatchRequestedCount();

  const statusTitle = state.isRunning
    ? `実行中 ${state.completedCount} / ${state.totalCount}半荘`
    : (state.lastSummaryText || "停止中");

  const statusText = state.isRunning
    ? (state.stopRequested
        ? "この半荘が終わったら停止します。左右CPは今の設定、自分は内部AIに固定して回しています。"
        : "左右CPは今の設定のまま、自分だけ内部AIに固定して自動で回しています。")
    : "開始時は自分を内部AIに固定し、終了時に元の操作モードへ戻します。";

  return `
    <div class="settingsSection settingsBatchPanel">
      <div class="settingsSectionTitle">自動対局バッチ</div>

      <div class="settingsField">
        <label class="settingsLabel" for="settingsBatchMatchCount">回す半荘数</label>
        <input
          type="number"
          class="settingsSelect"
          id="settingsBatchMatchCount"
          min="1"
          max="${AUTO_MATCH_BATCH_MAX_COUNT}"
          step="1"
      ...
      <div class="settingsBatchStatus">
        <strong>${escapeSettingsHtml(statusTitle)}</strong>
        ${escapeSettingsHtml(statusText)}
      </div>

      <div class="settingsBatchActions">
        <button
          type="button"
          class="settingsBatchBtn isPrimary"
          id="settingsBatchStartBtn"
          ${state.isRunning ? "disabled" : ""}
        >開始</button>
        <button
          type="button"
          class="settingsBatchBtn"
          id="settingsBatchStopBtn"
          ${state.isRunning ? "" : "disabled"}
        >停止予約</button>
      </div>
    </div>
...
function installAutoMatchBatchHooks(){
  try{
    if (typeof showHanchanEndOverlay !== "function") return;
    if (showHanchanEndOverlay.__autoMatchBatchWrapped) return;

    const originalShow = showHanchanEndOverlay;
    const wrappedShow = function(endInfo, settlement){
      const result = originalShow.apply(this, arguments);
      try{ handleAutoMatchBatchHanchanEnd(endInfo, settlement); }catch(e){}
      return result;
    };

    wrappedShow.__autoMatchBatchWrapped = true;
...
function buildCpuSettingsPaneHtml(){
  return `
    ...
    ${buildAutoMatchBatchSectionHtml()}
  `;
}
...
function renderGameSettingsPanel(){
  ...
  const batchCountInput = document.getElementById("settingsBatchMatchCount");
  if (batchCountInput){
    batchCountInput.addEventListener("change", ()=>{
      setAutoMatchBatchRequestedCount(batchCountInput.value);
      saveGameSettingsToStorage();
      renderGameSettingsPanel();
    });
  }

  const batchStartBtn = document.getElementById("settingsBatchStartBtn");
  if (batchStartBtn){
    batchStartBtn.addEventListener("click", ()=>{
      startAutoMatchBatchRun();
    });
  }

  const batchStopBtn = document.getElementById("settingsBatchStopBtn");
  if (batchStopBtn){
    batchStopBtn.addEventListener("click", ()=>{
      requestStopAutoMatchBatchRun();
      renderGameSettingsPanel();
    });
  }
}
...
function collectGameSettingsForStorage(){
  return {
    ...
    batch: {
      requestedMatchCount: getAutoMatchBatchRequestedCount()
    }
  };
}
...
function applyGameSettingsFromObject(raw){
  ...
  try{
    if (batch.requestedMatchCount != null){
      setAutoMatchBatchRequestedCount(batch.requestedMatchCount);
    }
  }catch(e){}
