...
function pushRiichiAutoDrawLogSafe(tile){
  try{
    if (!tile) return;
    ...
    window.MBSanmaMatchLog.pushEvent("draw", {
      seatIndex: 0,
      tile: window.MBSanmaMatchLog.cloneTile ? window.MBSanmaMatchLog.cloneTile(tile) : { code: tile.code, imgCode: tile.imgCode || tile.code },
      source: "wall"
    });
  }catch(e){}
}
...
function scheduleRiichiAuto(){
  ...
  if (!drawn){
    ...
    if (drawn){
      pushRiichiAutoDrawLogSafe(drawn);
    }
