...
const CPU_OPEN_PROFILE_LIBRARY = {
  ...
  balanced: {
    ...
    riichiDangerPassBonus: 4.8,
    noValuePassBonus: 2.0,
    farShantenPassBonus: 1.2,
    topScorePassBonus: 0.7,

    ponYakuhaiBonus: 3.2,
    ponTenpaiKeepBonus: 2.8,
    ponShantenAdvanceBonus: 1.6,
    ponImproveCountFactor: 0.05,
    ponWaitTypeFactor: 0.14,
    ponHonitsuBonus: 1.8,
    ponToitoiBonus: 1.8,
    ponTanyaoBonus: 0.9,
    ponAlreadyOpenBonus: 0.6,
    ponDealerBonus: 0.5,
    ponRiichiDangerPenalty: 4.8,
    ponNoValuePenalty: 3.6,
    ponSpeedOnlyPenalty: 1.6,
    ponWorsenPenalty: 4.6,
    ponFarShantenPenalty: 1.4,
    ponTopScorePenalty: 0.5,

    minkanYakuhaiBonus: 1.2,
    minkanTenpaiKeepBonus: 2.8,
    minkanShantenAdvanceBonus: 0.7,
    minkanImproveCountFactor: 0.04,
    minkanWaitTypeFactor: 0.26,
    minkanHonitsuBonus: 0.8,
    minkanToitoiBonus: 0.9,
    minkanRiichiDangerPenalty: 6.8,
    minkanNoValuePenalty: 4.2,
    minkanSpeedOnlyPenalty: 1.0,
    minkanWorsenPenalty: 5.6,
    minkanClosedHandPenalty: 0.8,
...
function getCpuOpenSeatProfileKey(seatIndex){
  if (seatIndex !== 1 && seatIndex !== 2) return "balanced";
...
function setCpuOpenSeatProfile(seatIndex, key){
  if (seatIndex !== 1 && seatIndex !== 2) return getCpuOpenSeatProfileKey(seatIndex);
...
function getCpuOpenSeatEngineMode(seatIndex){
  if (seatIndex !== 1 && seatIndex !== 2) return "internal";
...
function setCpuOpenSeatEngineMode(seatIndex, mode){
  if (seatIndex !== 1 && seatIndex !== 2) return getCpuOpenSeatEngineMode(seatIndex);
