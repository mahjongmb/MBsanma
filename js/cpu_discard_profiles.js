...
const CPU_DISCARD_PROFILE_LIBRARY = {
  ...
  balanced: {
    ...
    label: "Balanced",
    shantenWeight: 220,
    improveCountFactor: 4.2,
    drawnDiscardBonus: 0,
    riichiGenbutsuBonus: 32,
    riichiSujiBonus: 16,
    riichiOneChanceBonus: 7,
    riichiDangerPenalty: 18,
    openYakuhaiDangerPenalty: 9,
    riichiReadyBonus: 26,
    doraKeepBonus: 12,
    honorPairKeepBonus: 8,
    isolatedHonorDiscardBonus: 0,
...
function setCpuDiscardSeatExternalStyle(seatIndex, key){
  if (seatIndex !== 1 && seatIndex !== 2) return getCpuDiscardSeatExternalStyle(seatIndex);
...
function setCpuDiscardSeatProfile(seatIndex, key){
  if (seatIndex !== 1 && seatIndex !== 2) return getCpuDiscardSeatProfile(seatIndex);
...
function getCpuDiscardSeatEngineMode(seatIndex){
  if (seatIndex !== 1 && seatIndex !== 2) return "legacy";
...
function setCpuDiscardSeatEngineMode(seatIndex, mode){
  if (seatIndex !== 1 && seatIndex !== 2) return getCpuDiscardSeatEngineMode(seatIndex);
