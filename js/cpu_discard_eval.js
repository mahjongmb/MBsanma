...
function isCpuDiscardRedFiveTile(tile){
  ...
  const imgCode = typeof tile.imgCode === "string" ? tile.imgCode : "";
  return imgCode === "r5p" || imgCode === "r5s";
}
...
function evaluateCpuDiscardCandidate(snapshot, candidate, profile){
  ...
  pushCpuDiscardEvalPart(parts, "red_keep_bonus", afterRedCount * (Number(profile.redKeepBonus) || 0));

  if (isCpuDiscardRedFiveTile(candidate.discardTile)){
    pushCpuDiscardEvalPart(parts, "red_discard_penalty", -(Number(profile.redDiscardPenalty) || 0));
  }
