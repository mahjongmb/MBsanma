...
function cloneTileForDecisionSnapshot(tile){
  ...
  if (tile.imgCode) out.imgCode = tile.imgCode;
  if (tile.id != null) out.id = tile.id;
  if (tile.isRed) out.isRed = true;
...
function inferValuePlanHintsForDecisionSnapshot(seatIndex, tiles, meldsArr = [], extraCodes = []){
  ...
  const suitEntries = ["m","p","s"].map((suit)=> [suit, suitCounts[suit] | 0]);
  suitEntries.sort((a, b)=> b[1] - a[1]);
  const maxSuitCount = suitEntries[0] ? suitEntries[0][1] : 0;
  const secondSuitCount = suitEntries[1] ? suitEntries[1][1] : 0;
  const honorCount = suitCounts.z | 0;

  if (maxSuitCount >= 7 && secondSuitCount <= 1){
...
function buildCpuCallSnapshot(seatIndex, discardedTile, discarderSeatIndex, sourceType = "discard"){
  try{
    ...
    const legalPon = !isEnded && !(typeof isCpuRiichiSeat === "function" && isCpuRiichiSeat(seatIndex)) && sameTileCount >= 2;
