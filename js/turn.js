...
function pushCpuRiichiDeclareLogSafe(seatIndex, tile, extra){
  try{
    if (!tile) return;
    if (typeof window === "undefined" || !window.MBSanmaMatchLog || typeof window.MBSanmaMatchLog.pushEvent !== "function") return;
    const payload = {
      seatIndex,
      declareTile: window.MBSanmaMatchLog.cloneTile ? window.MBSanmaMatchLog.cloneTile(tile) : { code: tile.code, imgCode: tile.imgCode || tile.code }
    };
...
function cpuDoOneDiscard(seatIndex){
  ...
  if (best.willRiichi){
    pushCpuRiichiDeclareLogSafe(seatIndex, discardedTile, {
