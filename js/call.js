...
function getPendingCallFromSeatIndex(callInfo){
  const info = callInfo && typeof callInfo === "object" ? callInfo : pendingCall;
  if (!info || !info.from) return null;
  if (info.from === "R") return 1;
  if (info.from === "L") return 2;
  return null;
}

function getPendingCallJunme(callInfo){
  const info = callInfo && typeof callInfo === "object" ? callInfo : pendingCall;
  const fromSeatIndex = getPendingCallFromSeatIndex(info);
  if (fromSeatIndex === 1) return Array.isArray(cpuRightRiver) ? cpuRightRiver.length : 0;
  if (fromSeatIndex === 2) return Array.isArray(cpuLeftRiver) ? cpuLeftRiver.length : 0;
  return 0;
}

function pushCallPromptLogSafe(callInfo){
  const info = callInfo && typeof callInfo === "object" ? callInfo : pendingCall;
  if (!info) return;

  try{
    if (typeof window === "undefined" || !window.MBSanmaMatchLog || typeof window.MBSanmaMatchLog.pushEvent !== "function") return;
    window.MBSanmaMatchLog.pushEvent("call_prompt", {
      seatIndex: 0,
      fromSeatIndex: getPendingCallFromSeatIndex(info),
      junme: getPendingCallJunme(info),
      code: info.code || "",
      canPon: !!info.canPon,
      canRon: !!info.canRon,
      canMinkan: !!info.canMinkan,
      aiControlled: !!info.aiControlled
    });
  }catch(e){}
}

function pushCallPassLogSafe(reason, callInfo){
  const info = callInfo && typeof callInfo === "object" ? callInfo : pendingCall;
  if (!info) return;

  try{
    if (typeof window === "undefined" || !window.MBSanmaMatchLog || typeof window.MBSanmaMatchLog.pushEvent !== "function") return;
    window.MBSanmaMatchLog.pushEvent("call_pass", {
      seatIndex: 0,
      fromSeatIndex: getPendingCallFromSeatIndex(info),
      junme: getPendingCallJunme(info),
      code: info.code || "",
...
function beginCallPrompt(from, tile){
  ...
  pushCallPromptLogSafe(pendingCall);
  render();
...
function choosePon(doIt){
  ...
  if (!doIt){
    if (triggerCpuRonPriorityOnCpuDiscardSafe()){
      pushCallPassLogSafe("cpu_ron_priority");
      endCallPrompt("cpu_ron");
      return;
    }
    pushCallPassLogSafe("choose_pon_false");
    endCallPrompt("pass");
    return;
  }
  ...
  try{
    if (typeof window !== "undefined" && window.MBSanmaMatchLog && typeof window.MBSanmaMatchLog.pushEvent === "function"){
      window.MBSanmaMatchLog.pushEvent("call_pon", {
        seatIndex: 0,
        fromSeatIndex: calledSeatIndex,
        junme: getPendingCallJunme({ from, code }),
        code,
...
function chooseMinkan(doIt){
  ...
  if (!doIt){
    ...
    pushCallPassLogSafe("choose_minkan_false");
    endCallPrompt("pass");
    return;
  }
  ...
  try{
    if (typeof window !== "undefined" && window.MBSanmaMatchLog && typeof window.MBSanmaMatchLog.pushEvent === "function"){
      window.MBSanmaMatchLog.pushEvent("call_minkan", {
        seatIndex: 0,
        fromSeatIndex: calledSeatIndex,
        junme: getPendingCallJunme({ from, code }),
        code,
...
function chooseRon(doIt){
  ...
  if (!doIt){
    ...
    pushCallPassLogSafe("choose_ron_false");
    endCallPrompt("pass");
    return;
  }
