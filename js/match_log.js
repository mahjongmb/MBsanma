...
(function(global){
  ...
  function readStoredListFromLocalStorage(scopeKey){
    return sortStoredLogs(readLegacyStoredList(scopeKey));
  }

  function persistCompletedLogToLocalStorage(log, scopeKey){
    if (!log || typeof log !== "object") return false;
    const optimizedLog = buildStorageOptimizedLog(log);
    const storageKey = scopeKey || getScopedStorageKey();

    try{
      const list = readStoredListFromLocalStorage(storageKey);
      const next = sortStoredLogs([optimizedLog, ...list.filter((item)=> item.matchId !== optimizedLog.matchId)]).slice(0, MAX_STORED_MATCHES);
