...
function applyResultOverlayResponsiveLayout(){
  ...
  if (resultYakuListEl){
    if (compact){
      resultYakuListEl.style.gridTemplateColumns = "minmax(0, 1fr) minmax(0, 1fr)";
      resultYakuListEl.style.columnGap = "8px";
      resultYakuListEl.style.rowGap = "5px";
    } else {
  ...
  if (resultInfoRowEl){
    ...
    resultInfoRowEl.style.gridTemplateColumns = compact ? "minmax(0, 1fr) minmax(0, 1fr)" : "1fr 1fr";
...
function buildResultChipInfoByEntry(entryOverride){
  ...
  const baseChipUnits = Math.max(0, (colorChipCount | 0) + (uraChipCount | 0) + (ippatsuChipCount | 0));
  const delta = [0, 0, 0];
